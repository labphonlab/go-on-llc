...
- **アプリごとのサポートページ**は `/speechlab` `/gotan-ko` `/hanbun` `/osaat-en`。App Store のサポートURLがここを指すので、**パスは変えない**。中身は `src/components/AppSupport.astro` が共通で描画し、文面は `src/i18n/pages.ts`（`satisfies AppSupportCopy`）に置く。冒頭に開発中である旨を明記したうえで公開している（ナビには置かず、`/products` と `/support` の開発中一覧から辿る）。増やすときは pages.ts に文面を足し、薄いページを1枚作り、`catalog.ts` の `supportPath` とサイトマップに登録する。
