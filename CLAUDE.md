...
- **`/speechlab` は SpeechLab 専用のサポートページ**。App Store のサポートURLがここを指す。冒頭に開発中である旨を明記したうえで公開している（ナビには置かず、`/products` と `/support` の開発中一覧から辿る）。同種のページを足すときは `catalog.ts` の `supportPath` に登録すれば導線が付く。
